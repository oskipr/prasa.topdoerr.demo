...
const IntelligentAlerts = () => {
  ...
  return (
    <div>
      <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-semibold text-gray-800 mb-3 sm:mb-4">Alertas Inteligentes</h2>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-2 sm:space-x-3 md:space-x-4 flex-1 min-w-0">
                    <span className="text-xl sm:text-2xl flex-shrink-0">{getAlertIcon(alert.title)}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 mb-2 gap-2 sm:gap-0">
                        <h3 className="font-semibold text-gray-900 text-sm sm:text-base md:text-lg">{alert.title}</h3>
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium border ${badge.color} self-start sm:self-auto`}
                        >
                          {badge.text}
                        </span>
                      </div>
                      <p className="text-xs sm:text-sm md:text-base text-gray-600 mb-2">{alert.description}</p>
                      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 gap-1 sm:gap-0 text-xs text-gray-500">
